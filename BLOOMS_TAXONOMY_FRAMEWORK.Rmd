...
bloom_data <- data.frame(
  ...
  Count = c(2, 2, 5, 6, 4, 1),
  Percentage = c(10, 10, 25, 30, 20, 5),
  Target = c(13, 13, 20, 20, 27, 7),
  Order = 1:6
)

kable(bloom_data[, -5], caption = "Bloom's Taxonomy Distribution Analysis - Updated Exercise Order")
```
...
### LEVEL 1: FOUNDATION (Exercises 1-2) - Maximum Scaffolding 🌱

#### 01. Level of Measurement 
- **Bloom Level:** Remember
- **Scaffolding:** Complete definitions provided, clear examples
- **Context:** Crime type classification (nominal, ordinal, interval, ratio)
...
- **Context:** Police report analysis purposes - when to describe vs. when to infer

### LEVEL 2: UNDERSTANDING (Exercises 3-4) - Heavy Scaffolding 🌿
...
- **Context:** Juvenile delinquency incidents - impact of extreme values
...
### LEVEL 3: APPLICATION (Exercises 5-9) - Moderate Scaffolding �

#### 05. Crime Rate Calculation
- **Bloom Level:** Apply
- **Scaffolding:** Complete formula provided, step-by-step guidance
- **Context:** Standardizing burglary rates across populations
...
### LEVEL 4: ANALYSIS (Exercises 10-15) - Light-Moderate Scaffolding �
...
#### 15. Effect Size Calculation (Cohen's d)
- **Bloom Level:** Analyse
...
### LEVEL 5: EVALUATION (Exercises 16-19) - Light Scaffolding 🌺
...
scaffolding_levels <- data.frame(
  Exercise = 1:20,
  Level = c(rep("Maximum", 2), rep("Heavy", 2), rep("Moderate", 5), 
            rep("Light-Moderate", 6), rep("Light", 4), "Minimal"),
  Support_Score = c(rep(5, 2), rep(4, 2), rep(3, 5), rep(2, 6), rep(1, 4), 0)
)
...
progression_data <- data.frame(
  ...
  Exercises = c("1-2", "3-4", "5-9", "10-15", "16-19", "20"),
...
kable(progression_data, caption = "Learning Progression Overview - Updated Exercise Distribution")
```
